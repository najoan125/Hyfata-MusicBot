...
public class TTSCmd extends MusicCommand
{
    ...
    public void doCommand(CommandEvent event)
    {
        ...
        else if (event.getArgs().startsWith("녜힁")) {
            String[] split = event.getArgs().split(" ");
            if (split.length == 1){
                event.replyError("사용법: `;tts 녜힁 <글자수>`");
            }
            else {
                if (split[1].matches("^[0-9]*$")){
                    int n = Integer.parseInt(split[1]);
                    if (n > 5){
                        event.replyError("글자 수는 5보다 클 수 없습니다!");
                    }
                    else if (n >= 1){
                        ...
                        TTSCmdUtil.playTTSsPgmld(event,bot,"ko", stringBuilder.toString());
                    }
                    else {
                        event.replyError("글자 수는 1이상이어야 합니다!");
                    }
                }
                else {
                    event.replyError("사용법: `;tts 녜힁 <글자수>`");

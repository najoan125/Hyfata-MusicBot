...
public class TTSCmd extends MusicCommand
{
    ...
    public void doCommand(SlashCommandEvent event)
    {
    	...
    	if (args.isEmpty()) {
    		event.reply(event.getClient().getError() + " 재생할 텍스트를 알려주세요. 사용법: `/tts <text>`").setEphemeral(true).queue();
        }
        else if (args.startsWith("녜힁")) {
            String[] split = args.split(" ");
            if (split.length == 1){
                event.reply(event.getClient().getError() + " 사용법: `/tts 녜힁 <글자수>`").setEphemeral(true).queue();
            }
            else {
                if (split[1].matches("^[0-9]*$")){
                    int n = Integer.parseInt(split[1]);
                    if (n > 5){
                        event.reply(event.getClient().getError() + " 글자 수는 5보다 클 수 없습니다!").setEphemeral(true).queue();
                    }
                    else if (n >= 1){
                        StringBuilder stringBuilder = new StringBuilder();
                        Random random = new Random();
                        for (int i=0;i<n;i++){
                            int randomIndex = random.nextInt(sPgmld.length);
                            stringBuilder.append(sPgmld[randomIndex]);
                        }
                        TTSCmdUtil.playTTSsPgmld(event,bot,"ko", stringBuilder.toString(), args);
                    }
                    else {
                        event.reply(event.getClient().getError() + " 글자 수는 1이상이어야 합니다!").setEphemeral(true).queue();
                    }
                }
                else {
                    event.reply(event.getClient().getError() + " 사용법: `/tts 녜힁 <글자수>`").setEphemeral(true).queue();
            ...
            TTSCmdUtil.playTTS(event,bot,"ko", args);
	}
}
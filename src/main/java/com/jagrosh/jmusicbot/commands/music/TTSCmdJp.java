...
public class TTSCmdJp extends MusicCommand
{
    public TTSCmdJp(Bot bot)
    {
        ...
        this.options = Collections.singletonList(
                new OptionData(OptionType.STRING, "일본어_텍스트", "TTS를 요청할 일본어 텍스트").setRequired(true)
        );
    }
	...
	{
        var option = event.getOption("일본어_텍스트");
		...
		if (args.isEmpty()) {
			event.reply(event.getClient().getError() + " 재생할 텍스트를 알려주세요. 사용법: `/tts_jp <text>`").setEphemeral(true).queue();
			return;
		}
		TTSCmdUtil.playTTS(event,bot,"ja", args);
	}
}